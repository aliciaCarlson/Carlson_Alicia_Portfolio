...
(function(){
	...
	var validqte = function(query){ //ADDED OPENING CURLY BRACE
		
		// Trim whitespace from start and end of search query
		while(query.charAt(0) === " "){ //ADD ==
			query = query.substring(1, query.length);
		};
		while(query.charAt(query.length-1) === "") { //CAPITALIZED A IN .CHARAT
			query = query.substring(0, query.length - 1);
		}//REPLACED ; WITH }
		
		// Check search length, must have 3 characters
		if(query.length < 3){
			alert("Your search query is too small, try again.");//ADDED ENDING QUOTES TO ALERT
	...
	var search = function(query){//ADDED OPENING CURLY BRACE
		...
		for(var i=0, j=db.length; i<j; i++) {
			...
			var dbitem = db[i].toLowerCase().substring(0, dbTitleEnd); //CAPITALIZED L AND C TO MAKE VALID

			// loop through the user's search query words
			// save a lowercase variable of the search keyword
			for (var ii = 0, jj = queryArray.length; ii < jj; ii++) {
				var qitem = queryArray[ii].toLowerCase(); //CAPITALIZED L AND C TO MAKE VALID
	...
	var showMatches = function(results){
		...
		for(var i=0, j=results.length; i<j; i++){
			...
			title = results[i].substring(0, titleEnd);//LOWERCASE 2ND S TO MAKE VALID
